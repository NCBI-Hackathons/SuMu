...
gene_names <- names(mutation_matrix)
gene_names <- gene_names[gene_names != 'sample']
...
```{r}
auc(clin_df, "os_10y", glmfit_clingen)
```

```{r}
auc(clin_df, "os_10y", glmfit_clingen, h_gram = T)
```

```{r}
auc(clin_df, "os_10y", glmfit_clingen, roc_plot = T)
```
